...
*/
  ...
  d.SetId(id)
  fmt.Println("Id of new resourceVM set. %v", id)
...
*/
  ...
  if err != nil {
    fmt.Println("Failed to read VM specs, removing it. %v",d.Id())
    d.SetId("")
    return nil
  }
  name := resp["name"].(string)
  d.Set("name", name)
  fmt.Println("VM name set. %v %v", name, d.Id())
...
*/
  ...
  if err != nil {
    fmt.Println("Failed deleting. %T",err)
  }
  return err
}
...
func orderFromCatalog(resource_params map[string]string) (map[string]interface{}, error) {
  ...
  resp, err := apicall(path,"",nil)
  if err != nil {
    fmt.Println("Failed to get service_catalogs. %T",err)
    return resp, err
  }
  ...
  if err2 != nil {
    fmt.Println("Failed to orderFromCatalog. %T",err2)
  }
  return res2, err2
}
